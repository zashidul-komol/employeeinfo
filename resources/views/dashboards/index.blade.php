...
@stop
                          ...
                          <div class="col-xs-7">
                            ...
                            @endphp
                             {{Form::text('spouse_mobile',$spouseMobile, array('class' => 'form-control'))}}
                              {!! $errors->first('spouse_mobile', '<p class="text-danger">:message</p>' ) !!} 
                          </div>
                        </div>
                        @php
                              $spouseOccupation = '';
  ...
  <div class="row animated fadeInRight">
    <div class="col-sm-12">
        <h4 class="section-subtitle"><b>Do you have any relatives working in Dhaka Ice Cream Industries Limited or any sister concern of Pandughar Group ?</b>
              ...
              <div class="table-responsive">
                <table id="basic-table" class="data-table table table-striped nowrap table-hover" cellspacing="0" width="100%">
                    ...
                    <tbody>
                        ...
                        <tr>
                        <td>{{$data->name or ''}}</td>
                        <td>{{$data->organizations->organization or ''}}</td>
                        <td>{{$data->relationship or ''}}</td>
                        <td>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">Add Relatives Information</h4>
        </div>
        <div class="modal-body">
          <span id="form_result_Child"></span>
          {{ Form::model(request()->old(),array('route' => array('relationships.store'),'enctype'=>'multipart/form-data','class'=>'form-horizontal')) }}
          <form method="post" id="child_form" class="form-horizontal" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
              <label class="control-label col-md-4">Relative Name : </label>
              <div class="col-md-6">
            ...
            <div class="form-group">
              ...
              <div class="col-sm-6">
                ...
                {!! $errors->first('organization_id', '<p class="text-danger">:message</p>' ) !!}
              </div>
            </div>
            <div class="form-group">
              <label class="control-label col-md-4">Relationship : </label>
              <div class="col-md-6">
                {{Form::select('relationship',[''=>'Please Select Relationship--']+['Parents'=>'Parents','Elder Brother'=>'Elder Brother','Younger Brother'=>'Younger Brother','Elder Sister'=>'Elder Sister','Younger Sister'=>'Younger Sister','Cousin'=>'Cousin', 'Father-in-law'=>'Father-in-law' ,'Mother-in-law'=>'Mother-in-law' , 'Brother-in-law'=>'Brother-in-law','Sister-in-law'=>'Sister-in-law'],null,array('class' => 'form-control'))}}
                {!! $errors->first('relationship', '<p class="text-danger">:message</p>' ) !!}
              </div>
            </div>
            <br/>
...
<script>
  ...
  $('#create_RelativeInfo').click(function(){
    $('#formModalRelative').modal('show');
  });

  $('#relative_form').on('submit', function(event){
    event.preventDefault();
    if ($('#action').val() == 'Add') 
    {
        $.ajax({
            ...
            success:function(data)
            {
              ...
              {
                html = '<div class="alert alert-success">' + data.success + '</div>';
                $('#relative_form')[0].reset();
                $('#user_table').DataTable().ajax.reload();
              }
              $('#form_result_relative').html(html);
            }
        })
    }
  });  
